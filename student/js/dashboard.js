...
// The student portal's access state is controlled by portal-control.js.
// Do NOT treat users.status as a platform-access gate here: pending_admission
// must not cause a signed-in user to be logged out before Founder controls apply.
async function loadStudent(uid, authUser = auth.currentUser) {
    if (!uid) throw new Error("No signed-in student account was found.");
    ...
    if (!userSnap.exists()) throw new Error("Your account profile could not be found.");
    ...
    // Keep the dashboard usable while the account is pending admission.
    // Founder platform controls remain responsible for suspension/lockdown.
    return {
        ...userData,
        ...studentData,
        id: uid,
        uid,
        name: displayName,
        fullName: studentData.fullName || userData.fullName || displayName,
        email,
        admissionNumber: studentData.admissionNumber || userData.admissionNumber || "Pending",
        status: studentData.status || userData.status || "Pending"
    };
}
...
function renderProfile(student) {
    ...
    text("studentFullName", name);
    text("studentEmail", student.email);
    text("studentAdmission", `Admission: ${student.admissionNumber || "Pending"}`);
    text("courseCount", Number(stats.coursesEnrolled ?? 0));
    text("lessonCount", Number(stats.lessonsCompleted ?? 0));
    text("certificateCount", Number(stats.certificates ?? 0));
    text("progressPercent", `${progress}%`);
    text("overallProgress", `${progress}%`);
    text("studentXP", xp);
    text("studentLevel", level);
    text("streakDays", `${streak} Days`);
    text("learningStreak", `${streak} Day${streak === 1 ? "" : "s"} Streak`);
    text("badgeCount", `${badges.length} Badges`);
    ...
    if (xpProgress) xpProgress.style.width = `${(xp % 250) / 250 * 100}%`;
}
...
async function loadCourses(uid) {
    ...
    try {
        ...
        const results = await Promise.allSettled(enrollments.docs.map(async item => {
            ...
            const course = await getDoc(doc(db, "courses", enrollment.courseId));
        ...
        if (!courses.length) {
            container.innerHTML = `<div class="course-card"><h3>Courses temporarily unavailable</h3><p>Your enrollment exists, but course details could not be loaded.</p></div>`;
            return;
        }
        ...
        console.error("[SSA Dashboard] Courses failed:", error);
        container.innerHTML = `<div class="course-card"><h3>Courses temporarily unavailable</h3><p>Please try again shortly.</p></div>`;
    }
}
...
async function loadAnnouncements() {
    ...
    try {
        ...
        container.innerHTML = snap.docs.map(item => {
            const a = item.data();
...
async function loadDashboard(uid, authUser) {
    const student = await loadStudent(uid, authUser);
    renderProfile(student);

    // Expose the exact authenticated identity for other student-page modules.
    window.ssaCurrentStudent = Object.freeze(student);
...
let started = false;
function start() {
    if (started || location.pathname.split("/").pop() !== "dashboard.html") return;
    started = true;

    onAuthStateChanged(auth, async user => {
        if (!user) return;
        try {
            await loadDashboard(user.uid, user);
            console.log("🔥 SSA Dashboard loaded for:", user.uid, user.email || user.displayName || "student");
        } catch (error) {
            console.error("[SSA Dashboard] Failed:", error);
            text("studentName", user.displayName || "Student");
            text("studentEmail", user.email || "");
        }
    });
}

...
async function loadStudent(uid) {
    ...
    if (!snap.exists()) throw new Error("Your student profile has not been issued yet.");
    return snap.data();
}
...
function renderProfile(student) {
    ...
    const level = Number(student.level ?? Math.floor(xp / 250) + 1);
    ...
    text("studentEmail", student.email || auth.currentUser?.email || "");
...
async function loadActivity(uid) {
    ...
    if (!container) return;
    try {
        const snap = await getDocs(query(collection(db, "notifications"), where("userId", "==", uid), orderBy("createdAt", "desc"), limit(5)));
        if (snap.empty) return;
        container.innerHTML = snap.docs.map(item => {
            const n = item.data();
            return `<div class="activity-item"><div class="activity-icon"><i data-lucide="bell"></i></div><div><h4>${escapeHTML(n.title || "Notification")}</h4><p>${escapeHTML(n.message || "You have a new update.")}</p></div></div>`;
        }).join("");
        window.lucide?.createIcons();
    } catch (error) {
        console.error("[SSA Dashboard] Activity failed:", error);
    }
}
...
function start() {
    ...
    onAuthStateChanged(auth, user => {
        if (!user) return;
        loadDashboard(user.uid).catch(error => {
            console.error("[SSA Dashboard] Failed:", error);
            const name = $("studentName");
            if (name) name.textContent = "Student";
        });
    });
}

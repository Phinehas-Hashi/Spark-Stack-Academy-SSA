...
const state = { students:0, instructors:0, courses:0, revenue:0, payments:[], activity:[], admins:[] };
...
function refresh(){
  ...
  renderChart(); renderBrief(); renderActivity(); renderAdminDirectory();
}
...
function renderAdminDirectory(){
  const list=$("adminDirectoryList"); if(!list)return;
  const admins=state.admins;
  text("adminCount",admins.length.toLocaleString());
  list.replaceChildren();
  if(!admins.length){const empty=document.createElement("p");empty.className="founder-empty-state";empty.textContent="No administrator profiles found. Provision the first admin using the steps above.";list.appendChild(empty);return;}
  admins.forEach(admin=>{
    const row=document.createElement("article"); row.className="admin-directory-item";
    const name=admin.fullName||admin.name||"Administrator";
    const status=String(admin.status||"active").toLowerCase();
    const initials=name.split(/\s+/).filter(Boolean).slice(0,2).map(x=>x[0]).join("").toUpperCase()||"A";
    const avatar=document.createElement("span"); avatar.className="admin-directory-avatar"; avatar.textContent=initials;
    const info=document.createElement("div"); info.className="admin-directory-info";
    const strong=document.createElement("strong"); strong.textContent=name;
    const email=document.createElement("span"); email.textContent=admin.email||"No email recorded";
    const uid=document.createElement("small"); uid.textContent=`UID: ${admin.uid||"—"}`;
    info.append(strong,email,uid);
    const badge=document.createElement("b"); badge.className=`admin-status-badge ${status}`; badge.textContent=status;
    row.append(avatar,info,badge); list.appendChild(row);
  });
}
function bind(id,url){ $(id)?.addEventListener("click",()=>location.href=url); }
function openAdminModal(){
  const modal=$("createAdminModal"); if(!modal)return;
  modal.classList.add("show"); modal.setAttribute("aria-hidden","false");
  renderAdminDirectory(); window.lucide?.createIcons();
}
...
function bindAdminModal(){
  ...
  $("refreshAdminList")?.addEventListener("click",()=>{
    const button=$("refreshAdminList"); if(button){button.disabled=true;button.innerHTML='<i data-lucide="loader-circle"></i> Refreshing…';window.lucide?.createIcons();}
    setTimeout(()=>{renderAdminDirectory();if(button){button.disabled=false;button.innerHTML='<i data-lucide="refresh-cw"></i> Refresh Admin Directory';window.lucide?.createIcons();}},500);
  });
  document.addEventListener("keydown",e=>{if(e.key==="Escape"&&modal.classList.contains("show"))closeAdminModal();});
}
...
listen("users",snap=>{
  ...
  state.students=docs.filter(d=>d.role==="student").length;
  state.instructors=docs.filter(d=>d.role==="instructor").length;
  state.admins=docs.filter(d=>d.role==="admin").map(d=>({...d,uid:d.uid||d.id}));
  refresh();
});

...
async function setPortalState(target, suspended) {
    ...
    await writeControls({[target]: {...(current[target] || {}), suspended, updated_at: serverTimestamp(), updated_by: state.user.uid}}, `${label} portal ${suspended ? "suspended" : "restored"}`, suspended ? "portal_suspended" : "portal_restored", { target });
...
async function setLockdown(active) {
    ...
    await writeControls({
        lockdown: active,
        student: {...(current.student || {}), suspended: active, updated_at: serverTimestamp(), updated_by: state.user.uid},
        instructor: {...(current.instructor || {}), suspended: active, updated_at: serverTimestamp(), updated_by: state.user.uid}
    }, active ? "Emergency lockdown activated" : "Emergency lockdown lifted", active ? "lockdown_enabled" : "lockdown_disabled", { affects: ["student", "instructor"] });
...
function renderMaintenance(maintenance) {
    const element = $("scheduleInfo"); if (!element) return;
...
function renderControls(data = {}) {
    const studentSuspended = !!data.student?.suspended, instructorSuspended = !!data.instructor?.suspended, lockdown = !!data.lockdown;
...
function listenControls() { onSnapshot(controlRef(), snapshot => renderControls(snapshot.exists() ? snapshot.data() : {}), error => { console.error("Control listener failed:", error); toast("Unable to read platform controls.", "error"); }); }

function listenAuditLog() {
    const commandLog = $("commandLog"); if (!commandLog) return;
...
function bindEvents() {
    $("studentToggle").onclick = async () => { try { const snapshot = await getDoc(controlRef()); await setPortalState("student", !snapshot.data()?.student?.suspended); } catch (error) { console.error(error); toast(error.message || "Unable to change student portal state.", "error"); } };
    $("instructorToggle").onclick = async () => { try { const snapshot = await getDoc(controlRef()); await setPortalState("instructor", !snapshot.data()?.instructor?.suspended); } catch (error) { console.error(error); toast(error.message || "Unable to change instructor portal state.", "error"); } };
    $("lockdownToggle").onclick = async () => {
        try {
            const snapshot = await getDoc(controlRef()), active = !!snapshot.data()?.lockdown;
            if (!active) {
                const confirmed = await window.ssaConfirm("This will immediately restrict BOTH the Student and Instructor portals. Use this only for a genuine emergency.", {title:"Activate emergency lockdown?", confirmText:"Activate Lockdown", tone:"danger", icon:"⛔"});
                if (!confirmed) return;
            }
            $("lockdownToggle").disabled = true; await setLockdown(!active);

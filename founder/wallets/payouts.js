...
async function loadFinanceSecurity() {
  try {
    ...
    console.error("Finance security load failed:", e);
  }
}
...
async function resolveNames(items) {
  ...
  await Promise.all(ids.map(async uid => {
    ...
    try {
      ...
      names.set(uid, snap.exists()
        ? (snap.data().fullName || snap.data().name || snap.data().email || uid)
        : uid);
    } catch {
      names.set(uid, uid);
    }
...
function render() {
  ...
  const totals = {
    pending: [0, 0], approved: [0, 0], paid: [0, 0], rejected: [0, 0]
  };

  rows.forEach(x => {
    const status = String(x.status || "pending").toLowerCase();
    if (totals[status]) {
      totals[status][0] += Number(x.amount || 0);
      totals[status][1]++;
    }
  });
  ...
  const list = rows.filter(x => {
    ...
    const text = JSON.stringify({
      ...x,
      instructorName: names.get(x.instructorId) || ""
    }).toLowerCase();
  ...
  table.innerHTML = list.length
    ? list.map(x => {
        ...
        const actions = status === "pending"
          ? `<button class="action approve" data-a="approve" data-id="${esc(x.id)}">Approve</button>
             <button class="action reject" data-a="reject" data-id="${esc(x.id)}">Reject</button>`
          : status === "approved"
...
async function action(id, type) {
  ...
  if (financeSecurity.treasuryLocked) {
    throw new Error("Treasury is locked. Unlock it before processing payouts.");
  }
  ...
  if (["approve", "reject"].includes(type) && current !== "pending") {
    throw new Error("Only pending requests can be reviewed.");
  }
  if (type === "paid" && current !== "approved") {
  ...
  if (amount > Number(financeSecurity.withdrawalLimit || 50000)) {
    throw new Error(`Payout exceeds the configured limit of ${money(financeSecurity.withdrawalLimit)}.`);
  }
  ...
  if (status === "paid") {
    await runTransaction(db, async tx => {
      ...
      if (String(latest.status || "pending").toLowerCase() !== "approved") {
        throw new Error("This payout is no longer approved for settlement.");
      }

...
// Toasts + accessible modals + legacy prompt/confirm bridge
// ============================================================
...
(() => {
  ...
  function installStyles(){
    ...
    style.textContent=`
      ...
      .ssa-modal-field,.ssa-modal-select{width:100%;margin-top:14px;padding:12px 13px;border:1px solid #d8dee8;border-radius:11px;background:#fff;color:#172033;font:600 14px inherit;outline:none}.ssa-modal-field:focus,.ssa-modal-select:focus{border-color:#60a5fa;box-shadow:0 0 0 3px rgba(37,99,235,.1)}
  ...
  function toast(message,type="info",duration=3800){
    installStyles(); let root=document.getElementById("ssa-toast-root");
  ...
  function dialog({title="Are you sure?",message="",confirmText="Continue",cancelText="Cancel",tone="warning",icon="⚠",field=null,choices=null}){
    ...
    return new Promise(resolve=>{
      ...
      const modal=document.createElement("div");modal.className=`ssa-modal ${tone}`;modal.setAttribute("role","dialog");modal.setAttribute("aria-modal","true");
      ...
      const copy=document.createElement("div");const titleEl=document.createElement("h2");titleEl.className="ssa-modal-title";titleEl.textContent=title;const msg=document.createElement("p");msg.className="ssa-modal-message";msg.textContent=message;copy.append(titleEl,msg);
      let control=null;
      if(field){control=document.createElement("input");control.className="ssa-modal-field";control.value=field.value||"";control.placeholder=field.placeholder||"";control.type=field.type||"text";copy.append(control)}
      if(choices){control=document.createElement("select");control.className="ssa-modal-select";choices.forEach(choice=>{const option=document.createElement("option");option.value=choice.value;option.textContent=choice.label;control.appendChild(option)});copy.append(control)}
      head.append(iconEl,copy);
      const actions=document.createElement("div");actions.className="ssa-modal-actions";const cancel=document.createElement("button");cancel.className="ssa-modal-btn";cancel.type="button";cancel.textContent=cancelText;const confirm=document.createElement("button");confirm.className="ssa-modal-btn primary";confirm.type="button";confirm.textContent=confirmText;actions.append(cancel,confirm);modal.append(head,actions);backdrop.appendChild(modal);document.body.appendChild(backdrop);document.body.style.overflow="hidden";
      const previous=document.activeElement;const finish=value=>{document.removeEventListener("keydown",onKey);backdrop.remove();document.body.style.overflow="";previous?.focus?.();resolve(value)};const onKey=e=>{if(e.key==="Escape")finish(null);if(e.key==="Enter"&&document.activeElement===confirm)confirm.click()};cancel.onclick=()=>finish(null);confirm.onclick=()=>finish(control?control.value:true);document.addEventListener("keydown",onKey);(control||confirm).focus();
    });
  }

  let lastClickTarget=null;
  let replayConfirm=false;
  let replayPromptValue=undefined;
  document.addEventListener("click",e=>{lastClickTarget=e.target.closest("button,a,[role='button'],input[type='button'],input[type='submit']")||e.target},{capture:true,passive:true});
  ...
  window.confirm=message=>{
    if(replayConfirm){replayConfirm=false;return true;}
    const target=lastClickTarget;
    window.ssaConfirm(String(message||"Are you sure?"),{title:"Confirm action",confirmText:"Continue",tone:"warning",icon:"⚠"}).then(ok=>{
      if(ok&&target?.isConnected){replayConfirm=true;target.click();}
    });
    return false;
  };
  window.prompt=(message,defaultValue="")=>{
    if(replayPromptValue!==undefined){const value=replayPromptValue;replayPromptValue=undefined;return value;}
    const target=lastClickTarget;
    window.ssaPrompt(String(message||"Enter a value"),{value:defaultValue}).then(value=>{
      if(value!==null&&target?.isConnected){replayPromptValue=value;target.click();}
    });
    return null;
  };

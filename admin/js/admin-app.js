...
const fetchComponent = async path => { const response = await fetch(path); if (!response.ok) throw new Error(`Failed to load ${path} (${response.status})`); return response.text(); };
...
async function getAdminProfile(uid) {
  ...
  catch (error) { console.error("Admin profile failed:", error); return null; }
}

function isAuthorized(profile) { return String(profile?.role || "").trim().toLowerCase() === "admin"; }
function syncAdminProfile(profile) { window.AdminSidebar?.updateProfile?.(profile); window.AdminTopbar?.updateProfile?.(profile); }
function denyAccess() { document.body.innerHTML = `<main class="admin-access-denied"><div class="access-denied-content"><div class="access-denied-icon">🛡️</div><h1>Access Restricted</h1><p>You don't have permission to access the Spark Stack Academy admin console.</p><a href="../login.html" class="access-denied-btn">Return to Login</a></div></main>`; }
async function logoutAdmin() { try { await signOut(auth); window.location.href = "../login.html"; } catch (error) { console.error("Logout failed:", error); } }

document.addEventListener("click", event => { const button = event.target.closest("#adminLogoutBtn, #adminDropdownLogout"); if (!button) return; event.preventDefault(); logoutAdmin(); });
window.AdminApp = { getProfile: getAdminProfile, logout: logoutAdmin, isAuthorized, getCurrentUser: () => auth.currentUser };

async function bootAdmin() {
  if (!(await loadAdminShell())) return;
